...
public class Laboratorio8Controller {
	...
	//private static final java.util.logging.Logger Logger = java.util.logging.Logger.getLogger(Laboratorio8Controller.class.getName());
	...
	public ModelAndView saveCliente(@Valid @ModelAttribute("cliente") Cliente c, BindingResult r){
		...
		if(r.hasErrors()){
			...
			if(c.getCcliente()==null){
				...
				key = clienteService.isertClienteAutoId(c);
			}
			else {
				mav.addObject("resultado",1);
    ...
    public ModelAndView insercionBatch() throws ParseException, java.text.ParseException {
	    ModelAndView mav = new ModelAndView();
